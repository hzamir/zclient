...
// 2019 - converting to es6
import * as th from "./th-utils";
...
export const fsm = {}; // create a new place holder for all the fsm datatypes

export function FsmFactory(collaboratorclass)
{
    ...
    const {states:srcStates, start:initialState, transitions:srcTransitions, options, eventSources=null} = config;
    ...
    this.graph = {
        ...
        ,initialState                 // name of the initial state
        ,srcStates                    // src objects/strings for building sobs
        ,srcTransitions               // source data for all the transitions
        ,srcInputs:config.io          // as specified
        ,eventSources
        ,options                      // statemachine wide configuration info
...
FsmFactory.prototype.markTerminalNodes= function()
{
    ...
    for(let i = 0; i < trans.length; ++i)
    {
        ...
        if(transition.from === '*') {
            ...
            for(let j = 0; j < fstates.length; ++j) {
                st = fstates[j];
                wayOutMap[st] = 1; // there is a way out
            }
        } else {
...
FsmFactory.prototype.compileStates = function(srcStates, options)
{
    ...
    if(!this.taskClass.prototype)
        throw new Error('taskClass is not a class: ' + th.funcname2(this.taskClass));
...
FsmFactory.prototype._visualizeTransitions = function(nodeNameMap, edges)
{
    ...
    for(let i = 0; i < trans.length; ++i)  {
        const tsrc = trans[i];
        if(tsrc.from === '*') {
            const temp = cloneProperties(tsrc);
            const states = this.graph.srcStates;
            for(let j = 0; j < states.length; ++j) {
                if(tsrc.to !== states[j]) {
                    temp.from = states[j];
...
FsmFactory.prototype._visualizeTransition = function(transition, nodeNameMap, edges, ephemeral)
{
    ...
    if(th.isNumber(transition.timer)) {
        const secs = (event / 1000);
        event = `${secs.toFixed(2)} secs`;
    }
    ...
    const colstr = ephemeral? ' color=grey':'';
    edges.push(`${from}->${to} [label="${event}${cond}"${colstr}]`);
};
